...
│                        浏览器 (Client)                       │
│  ┌─────────────┐  ┌──────────────┐  ┌────────────────────┐  │
│  │ React SPA   │  │ CodeMirror 6 │  │ Zustand Store      │  │
│  │ (路由/页面)  │  │ (编辑器内核)  │  │ (状态管理+持久化)   │  │
│  └──────┬──────┘  └──────┬───────┘  └─────────┬──────────┘  │
│         │                │                     │             │
│         └────────────────┴─────────────────────┘             │
│                          │                                   │
...
用户输入 Markdown
       │
       ▼
 CodeMirror 编辑器 ──onChange──▶ Zustand Store ──persist──▶ localStorage
       │                              │
       │                              ▼
       │                        marked 解析
       │                              │
       ...
       │                         预览面板渲染
       │
       │── 导出 HTML/MD ──▶ 前端 file-saver 直接下载
       │
       └── 导出 Word/Excel/PDF ──▶ POST /api/export
                                        │
                               ┌────────┼────────┐
                               ▼        ▼        ▼
                            Word     Excel      PDF
                            docx    exceljs  Puppeteer
                               │        │        │
                               └────────┼────────┘
                                        ▼
                                  二进制流返回浏览器
